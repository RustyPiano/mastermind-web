...
- **测试**：`npm test`（Vitest，当前 138 个测试，全部通过）
...
│   ├── daily.js                # dateToChallengeKey、hashStringToSeed、createSeededRng、generateDailySecret、getPreviousDayKey
│   ├── stats.js                # recordGameResult、hasCompletedDaily、getAverageRounds、isConsecutiveDay、补签卡逻辑
...
└── tests/
    ...
    ├── daily.test.js           # 18 个测试：每日挑战确定性、日期/哈希/RNG、入口状态文案
    ├── stats.test.js           # 27 个测试：统计、连胜、补签卡、预设分桶、防重复、上限
    ├── achievements.test.js    # 21 个测试：FIRST_TRY / LAST_CHANCE / BLIND 解锁逻辑
    ├── storage.test.js         # 11 个测试：序列化/恢复/版本校验
    ├── share.test.js           # 13 个测试：分享文本、移动端原生分享 payload、站点 URL 透传
    ├── mode-config.test.js     # 4 个测试：模式参数
    ├── analytics.test.js       # 7 个测试：埋点辅助函数与设备识别
    ├── guidance.test.js        # 5 个测试：状态/轮次提示文案
    ├── result-stats.test.js    # 1 个测试：结果弹窗统计文案
...
`getPreviousDayKey(challengeKey)` — 计算前一天的 challenge key，供补签卡与跨日连胜判断复用。
...
```js
{
  ...
  dailyResults: { [challengeKey]: { status, rounds, finishedAt } },
  achievements: string[],        // 解锁的成就 ID 数组，如 'FIRST_TRY', 'LAST_CHANCE', 'BLIND'
  makeupCards: { available, refreshMonth },
  makeupDays: string[],
}
```
...
- 模式选择页每日卡片可额外出现 `补签卡 banner`：仅在“今天未完成官方每日挑战”且“昨天刚好断签”时展示；若本月仍有补签卡，显示“使用补签卡”按钮；若昨天已补签，则显示“已补签 ✓”提示并隐藏按钮
...
- 若昨天断签但该日已通过补签卡登记，且今天胜利，则视为连续，继续累加 `currentDailyWin`
...
**补签卡逻辑**：
- 每月自动补充 `2` 张补签卡，记录在 `makeupCards.available`
- `makeupCards.refreshMonth` 记录最近一次补充发生的月份（格式 `YYYY-MM`）
- `getMissedDayKey(stats, challengeKey)` 只会在“今天距离上次完成刚好差 2 天”时返回昨天的 key
- `applyMakeupCard(stats, missedKey)` 会消耗 1 张补签卡，并把该日期写入 `makeupDays`
- 同一天重复补签是幂等的，不会重复扣卡
- 补签只影响“明天打赢后连胜能否续上”，不会把被补签那天写成一次真实 `dailyResults` 通关记录
...
| `updateMakeupCardBanner(...)` | 更新每日模式卡上的补签 banner、按钮和剩余张数 |
...
| 补签 banner（动态） | `${missedKey} 未参与每日挑战，可补签维持连击` / `已补签 ${missedKey} ✓ 今天通关即可延续连击` |
| 补签按钮 | 使用补签卡 |
| 补签附属文案（动态） | `剩余 N 张 · 每月补充` / `剩余 0 张 · 每月补充（已用完）` |
| 单人卡片标签 | 难度闯关 |
| 单人按钮 | 单人闯关 |
...
[screenMode]
  ├─ 若昨天刚好断签且今天未完成 → 显示补签 banner
  │   └─ 点"使用补签卡" → applyMakeupCard() → 保存 stats → 刷新首页文案
...
| 每日补签功能未在项目文档中体现 | 补充补签卡规则、数据结构、首页 banner 与测试矩阵 | `AGENTS.md`, `README.md` |
...
```
tests/analytics.test.js      7 个 — 设备识别、埋点辅助函数、daily practice 埋点区分
tests/engine.test.js        16 个 — calcFeedback（含 5 槽）、generateSecret（含 8 色 / 5 槽）、isWinningFeedback
tests/challenge.test.js     14 个 — 挑战 URL 编码/解析、挑战结果分享标题、旧 payload 向后兼容、挑战来源元信息
tests/daily.test.js         18 个 — dateToChallengeKey、倒计时、入口状态文案、generateDailySecret、isDailySessionForKey
tests/stats.test.js         27 个 — 首局初始化、预设分桶、连续连胜、失败归零、补签卡、365 条上限
tests/achievements.test.js  21 个 — FIRST_TRY / LAST_CHANCE / BLIND 解锁、去重、边界条件
tests/storage.test.js       11 个 — 序列化/恢复，旧 session 补位，版本校验，显式清除，默认值，normalizeStats
tests/share.test.js         13 个 — 分享文本、挑战邀请文案、移动端原生分享 payload、显式传递站点 URL
tests/guidance.test.js       5 个 — 初始提示、轮后总结、daily practice 状态文案
tests/mode-config.test.js    4 个 — 单人预设参数、重复规则、预设列表
tests/result-stats.test.js   1 个 — 结果弹窗统计区文案
